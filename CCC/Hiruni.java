...
public class Hiruni {
    ...
    public static void reportManagement() {
        ...
        if (regNoArray.length == 0) {
            System.out.println("\tNo student records found in system.");
        } else {
            System.out.println("--------------------------------------------------------------------------");
            System.out.printf("%-15s %-15s %-10s %-10s %-8s\n", "Reg No", "Name", "PRF", "DBMS", "GPA");
            System.out.println("--------------------------------------------------------------------------");

            for (int i = 0; i < regNoArray.length; i++) {
                String s1 = (prfMarksArray[i] >= 0) ? String.valueOf(prfMarksArray[i]) : (prfMarksArray[i] == -1 ? "ABS" : "N/A");
                String s2 = (dbmsMarksArray[i] >= 0) ? String.valueOf(dbmsMarksArray[i]) : (dbmsMarksArray[i] == -1 ? "ABS" : "N/A");
                System.out.printf("%-15s %-15s %-10s %-10s %-8.2f\n", regNoArray[i], nameArray[i], s1, s2, calculateGPA(i));
            }
            System.out.println("--------------------------------------------------------------------------");
        }
